...
@section('title', 'Trashed attribute_values')

@section('breadcrumb')
    @parent
    <li class="breadcrumb-item">attribute_values</li>
...
<div class="mb-5">
    <a href="{{ route('dashboard.categories.index') }}" class="btn btn-sm btn-outline-primary">{{__('app.back')}}</a>
</div>
...
<table class="table">
    ...
    <tbody>
        ...
        @empty
        <tr>
            <td colspan="7">{{__('app.no_attributes_defined')}}</td>
        </tr>
        @endforelse
    </tbody>
</table>

{{ $attributes_values->withQueryString()->appends(['search' => 1])->links() }}
